...
class GetSkuInventoryResult {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['unpaid_hold'])){
				
				$this->unpaid_hold = $vals['unpaid_hold'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("unpaid_hold" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->unpaid_hold); 
	...
	public function write($output){
		...
		if($this->unpaid_hold !== null) {
			
			$xfer += $output->writeFieldBegin('unpaid_hold');
			$xfer += $output->writeI32($this->unpaid_hold);
