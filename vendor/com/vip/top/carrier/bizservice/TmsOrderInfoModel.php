...
class TmsOrderInfoModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['exchangeForSn'])){
				
				$this->exchangeForSn = $vals['exchangeForSn'];
			}
			...
			if (isset($vals['ltLjrzTime'])){
				
				$this->ltLjrzTime = $vals['ltLjrzTime'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("exchangeForSn" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->exchangeForSn);
			...
			if ("ltLjrzTime" == $schemeField){
				
				$needSkip = false;
				$input->readI64($this->ltLjrzTime);
	...
	public function write($output){
		...
		if($this->exchangeForSn !== null) {
			
			$xfer += $output->writeFieldBegin('exchangeForSn');
			$xfer += $output->writeString($this->exchangeForSn);
		...
		if($this->ltLjrzTime !== null) {
			
			$xfer += $output->writeFieldBegin('ltLjrzTime');
			$xfer += $output->writeI64($this->ltLjrzTime);
