...
class ReportDeliveryRequest {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['deliveries'])){
				
				$this->deliveries = $vals['deliveries'];
			}
			...
			if ("deliveries" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem0 = new \com\vip\vop\logistics\carrier\service\ReportDeliveryInfo();
						$elem0->read($input);
						
						$this->deliveries[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('deliveries');
		
		if (!is_array($this->deliveries)){
			
			throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
		}
		
		$output->writeListBegin();
		foreach ($this->deliveries as $iter0){
