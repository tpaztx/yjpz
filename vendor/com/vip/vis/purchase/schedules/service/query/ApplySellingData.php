...
class ApplySellingData {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['isCrossPickUp'])){
				
				$this->isCrossPickUp = $vals['isCrossPickUp'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("followPoAssign" == $schemeField){
				...
				$needSkip = false;
				$input->readByte($this->followPoAssign); 
			...
			if ("isCrossPickUp" == $schemeField){
				
				$needSkip = false;
				$input->readByte($this->isCrossPickUp); 
	...
	public function write($output){
		...
		if($this->followPoAssign !== null) {
			...
			$xfer += $output->writeByte($this->followPoAssign);
		...
		if($this->isCrossPickUp !== null) {
			
			$xfer += $output->writeFieldBegin('isCrossPickUp');
			$xfer += $output->writeByte($this->isCrossPickUp);
