...
class CouponCommonModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['mobilePlatformAge'])){
				
				$this->mobilePlatformAge = $vals['mobilePlatformAge'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("mobilePlatformAge" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->mobilePlatformAge); 
	...
	public function write($output){
		...
		if($this->mobilePlatformAge !== null) {
			
			$xfer += $output->writeFieldBegin('mobilePlatformAge');
			$xfer += $output->writeI32($this->mobilePlatformAge);
