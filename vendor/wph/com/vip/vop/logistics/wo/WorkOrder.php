...
class WorkOrder {
	...
	public $expect_eply_time = null;
	public $standard_finish_time = null;
	public $back_logistics_nos = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			16 => array(
			'var' => 'expect_eply_time'
			),
			17 => array(
			'var' => 'standard_finish_time'
			),
			18 => array(
			'var' => 'back_logistics_nos'
			),
			19 => array(
			'var' => 'goods_list'
			),
		...
		if (is_array($vals)){
			...
			if (isset($vals['expect_eply_time'])){
				
				$this->expect_eply_time = $vals['expect_eply_time'];
			}
			
			
			if (isset($vals['standard_finish_time'])){
				
				$this->standard_finish_time = $vals['standard_finish_time'];
			}
			
			
			if (isset($vals['back_logistics_nos'])){
				
				$this->back_logistics_nos = $vals['back_logistics_nos'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("expect_eply_time" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->expect_eply_time);
			...
			if ("standard_finish_time" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->standard_finish_time);
			...
			if ("back_logistics_nos" == $schemeField){
				
				$needSkip = false;
				
				$this->back_logistics_nos = array();
				$_size5 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$this->back_logistics_nos[$_size5++] = $elem5;
					}
					catch(\Exception $e){
			...
			if ("goods_list" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem6 = new \com\vip\vop\logistics\wo\WorkOrderGoods();
						$elem6->read($input);
						
						$this->goods_list[$_size6++] = $elem6;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->expect_eply_time !== null) {
			
			$xfer += $output->writeFieldBegin('expect_eply_time');
			$xfer += $output->writeString($this->expect_eply_time);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->standard_finish_time !== null) {
			
			$xfer += $output->writeFieldBegin('standard_finish_time');
			$xfer += $output->writeString($this->standard_finish_time);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->back_logistics_nos !== null) {
			
			$xfer += $output->writeFieldBegin('back_logistics_nos');
			
			if (!is_array($this->back_logistics_nos)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->back_logistics_nos as $iter0){
