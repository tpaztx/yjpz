...
class RbpBaseStandardIntRespModel {
	
	static $_TSPEC;
	public $standardList = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['standardList'])){
				
				$this->standardList = $vals['standardList'];
			}
	...
	public function getName(){
		...
		return 'RbpBaseStandardIntRespModel';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("standardList" == $schemeField){
				
				$needSkip = false;
				
				$this->standardList = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem0 = new \com\vip\fcs\osp\rbp\service\RbpBaseStandardIntModel();
						$elem0->read($input);
						
						$this->standardList[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->standardList !== null) {
			...
			$xfer += $output->writeFieldBegin('standardList');
			...
			foreach ($this->standardList as $iter0){
