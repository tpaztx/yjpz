...
class CouponData {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['actUserTags'])){
				
				$this->actUserTags = $vals['actUserTags'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("actUserTags" == $schemeField){
				
				$needSkip = false;
				
				$this->actUserTags = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		if($this->actUserTags !== null) {
			
			$xfer += $output->writeFieldBegin('actUserTags');
			
			if (!is_array($this->actUserTags)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->actUserTags as $iter0){
