...
class PmsActCoreModel {
	...
	public $planId = null;
	public $isQuota = null;
	public $platformAge = null;
	public $shortLabel = null;
	public $businessCodeTmp = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['platformAge'])){
				
				$this->platformAge = $vals['platformAge'];
			}
			
			
			if (isset($vals['shortLabel'])){
				
				$this->shortLabel = $vals['shortLabel'];
			}
			
			
			if (isset($vals['businessCodeTmp'])){
				
				$this->businessCodeTmp = $vals['businessCodeTmp'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("platformAge" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->platformAge); 
			...
			if ("shortLabel" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->shortLabel);
			...
			if ("businessCodeTmp" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->businessCodeTmp);
	...
	public function write($output){
		...
		if($this->platformAge !== null) {
			
			$xfer += $output->writeFieldBegin('platformAge');
			$xfer += $output->writeI32($this->platformAge);
		...
		if($this->shortLabel !== null) {
			...
			$xfer += $output->writeFieldBegin('shortLabel');
			$xfer += $output->writeString($this->shortLabel);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->businessCodeTmp !== null) {
			
			$xfer += $output->writeFieldBegin('businessCodeTmp');
			$xfer += $output->writeString($this->businessCodeTmp);
