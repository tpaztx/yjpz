...
class SyncResultInfo {
	...
	public $syncFlag = null;
	public $syncMemo = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['syncFlag'])){
				
				$this->syncFlag = $vals['syncFlag'];
			}
			
			
			if (isset($vals['syncMemo'])){
				
				$this->syncMemo = $vals['syncMemo'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("syncFlag" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->syncFlag); 
			...
			if ("syncMemo" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->syncMemo);
	...
	public function write($output){
		...
		if($this->syncFlag !== null) {
			...
			$xfer += $output->writeFieldBegin('syncFlag');
			$xfer += $output->writeI32($this->syncFlag);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->syncMemo !== null) {
			
			$xfer += $output->writeFieldBegin('syncMemo');
			$xfer += $output->writeString($this->syncMemo);
