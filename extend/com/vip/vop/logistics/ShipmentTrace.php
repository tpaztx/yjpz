...
class ShipmentTrace {
	...
	public $next_site_longtitude = null;
	public $next_site_latitude = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			17 => array(
			'var' => 'next_site_longtitude'
			),
			18 => array(
			'var' => 'next_site_latitude'
			),
		...
		if (is_array($vals)){
			...
			if (isset($vals['next_site_longtitude'])){
				
				$this->next_site_longtitude = $vals['next_site_longtitude'];
			}
			
			
			if (isset($vals['next_site_latitude'])){
				
				$this->next_site_latitude = $vals['next_site_latitude'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("next_site_longtitude" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->next_site_longtitude);
			...
			if ("next_site_latitude" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->next_site_latitude);
	...
	public function write($output){
		...
		if($this->next_site_longtitude !== null) {
			
			$xfer += $output->writeFieldBegin('next_site_longtitude');
			$xfer += $output->writeString($this->next_site_longtitude);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->next_site_latitude !== null) {
			
			$xfer += $output->writeFieldBegin('next_site_latitude');
			$xfer += $output->writeString($this->next_site_latitude);
