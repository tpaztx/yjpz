...
class WpcChannelGoodsDetailVO {
	...
	public $isMp = null;
	public $catIdOne = null;
	public $catIdTwo = null;
	public $catIdThree = null;
	public $goodProps = null;
	public $wearReport = null;
	public $noStockAreaList = null;
	public $isAllNoStock = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['catIdOne'])){
				
				$this->catIdOne = $vals['catIdOne'];
			}
			
			
			if (isset($vals['catIdTwo'])){
				
				$this->catIdTwo = $vals['catIdTwo'];
			}
			...
			if (isset($vals['goodProps'])){
				
				$this->goodProps = $vals['goodProps'];
			}
			
			
			if (isset($vals['wearReport'])){
				
				$this->wearReport = $vals['wearReport'];
			}
			
			
			if (isset($vals['noStockAreaList'])){
				
				$this->noStockAreaList = $vals['noStockAreaList'];
			}
			
			
			if (isset($vals['isAllNoStock'])){
				
				$this->isAllNoStock = $vals['isAllNoStock'];
			}
	...
	public function getName(){
		...
		return 'WpcChannelGoodsDetailVO';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("catIdOne" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->catIdOne);
			...
			if ("catIdTwo" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->catIdTwo);
			...
			if ("goodProps" == $schemeField){
				
				$needSkip = false;
				
				$this->goodProps = array();
				$_size5 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem5 = new \com\vip\wpc\ospservice\goods\vo\WpcGoodsDetailPropVo();
						$elem5->read($input);
						
						$this->goodProps[$_size5++] = $elem5;
					}
					catch(\Exception $e){
			...
			if ("wearReport" == $schemeField){
				
				$needSkip = false;
				
				$this->wearReport = new \com\vip\wpc\ospservice\channel\vo\WpcChannelWearReportVo();
				$this->wearReport->read($input);
			...
			if ("noStockAreaList" == $schemeField){
				
				$needSkip = false;
				
				$this->noStockAreaList = array();
				$_size6 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem6 = new \com\vip\wpc\ospservice\channel\vo\WpcChannelNoStockAreaVo();
						$elem6->read($input);
						
						$this->noStockAreaList[$_size6++] = $elem6;
					}
					catch(\Exception $e){
			...
			if ("isAllNoStock" == $schemeField){
				
				$needSkip = false;
				$input->readBool($this->isAllNoStock);
	...
	public function write($output){
		...
		if($this->catIdOne !== null) {
			...
			$xfer += $output->writeFieldBegin('catIdOne');
			$xfer += $output->writeString($this->catIdOne);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->catIdTwo !== null) {
			
			$xfer += $output->writeFieldBegin('catIdTwo');
			$xfer += $output->writeString($this->catIdTwo);
		...
		if($this->goodProps !== null) {
			
			$xfer += $output->writeFieldBegin('goodProps');
			
			if (!is_array($this->goodProps)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->goodProps as $iter0){
		...
		if($this->wearReport !== null) {
			
			$xfer += $output->writeFieldBegin('wearReport');
			
			if (!is_object($this->wearReport)) {
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->wearReport->write($output);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->noStockAreaList !== null) {
			
			$xfer += $output->writeFieldBegin('noStockAreaList');
			
			if (!is_array($this->noStockAreaList)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->noStockAreaList as $iter0){
		...
		if($this->isAllNoStock !== null) {
			
			$xfer += $output->writeFieldBegin('isAllNoStock');
			$xfer += $output->writeBool($this->isAllNoStock);
